...
> **文档状态：已归档。** 本文记录项目早期状态，部分“未完成”描述已经失效，不得作为当前开发依据。请阅读 [`../README.md`](../README.md) 和 [`../PROJECT_STATUS.md`](../PROJECT_STATUS.md)。
...
目前已经完成的是“单轮 Agent 决策”和事件状态历史保存。评论传播、舆情指标和多轮模拟仍属于待实现功能。
...
- 读取 `state/event_state_history.json`；
...
本模块不提供本地模板兜底。API 请求失败或返回格式不正确时直接报错，
避免把非模型生成的评论混入实验结果。输入也可以包含当前官方声明，
用于生成声明发布后的候选评论。
...
以下文件中，`event_state_updater.py` 已有基础规则实现，其余模块仍是待实现接口。
...
负责保存每个时间步已经确定的事件状态，不负责预测或计算下一轮状态。

需要更新的字段包括：

- `official_statement`：人工或实验策略输入的官方声明；
- `official_statement_status`：人工选择的声明类型；
- `step`：根据历史记录自动递增；
- `event_id`：从固定事件信息中读取并校验。

事件历史记录不再保存 `group_conflict` 和 `seriousness`。

当前状态：已实现事件状态校验、历史追加和最新状态同步。
...
### `state/event_state_history.json`

保存每个时间步的动态事件信息，数组中每个对象对应一个时间步，例如：
...
- `state/event_state_history.json`：直接保存所有时间步的事件状态记录，数组中每个对象对应一个时间步。
