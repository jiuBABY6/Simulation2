...
- 适应性画像、社交网络、Agent影响力和固定进场时机实验仍属于后续扩展，不应与数据库和后端第一阶段混合开发。
- 代码交给团队前仍需完成密钥环境变量化和可复现依赖清单，真实密钥不得进入共享代码。

### 下一步唯一工作

完成团队交接前的配置安全和环境固化：将LLM密钥改为环境变量，增加环境示例和依赖清单，并用现有问界事件与高阳商场冲突事件执行最小启动检查。完成后冻结当前仿真核心接口，按照 [`TASK_BREAKDOWN.md`](./TASK_BREAKDOWN.md) 分配数据库、后端、前端和仿真验证任务。
...
当前 Demo 的主要输入包括事件材料、公众人物画像、官方回应方案和实验运行参数。基础 Persona 从 `output/personas` 只读加载；每次实验创建新的批次目录，保存事件、初始评论池和官方回应方案快照，避免覆盖历史结果。
...
公共黑板的评论表示当前平台上的舆论环境，是全局候选评论集合。

模拟信息茧房：

1. 读取全局评论池
2. 删除当前 Agent 自己已经选择的评论
3. 从剩余评论中随机抽取固定数量
4. 作为该 Agent 本轮看到的其他社会声音

### Agent决策

每个 Agent 每轮读取事件内容、当前官方声明及时间信息、个人可见评论、上一轮状态和个人历史评论。LLM只负责形成情绪、官方态度、评论意愿和评论派系等认知判断，最终表达仍由本地决策引擎完成筛选、评分和可复现选择。

个人可见评论默认最多 15 条，优先包含当前轮新评论 10 条和历史评论 5 条；数量不足时两层相互补足。Agent 自己此前所有实际表达作为个人历史参与后续判断，但不会再次被选为新的表达。
...
当前实验包含不回应、事实通报、共情安抚、辟谣澄清和处置进展五个场景。五个场景共享相同的事件、初始评论池和进场前状态，在官方触发进场后从同一状态快照分叉，分别保存在独立目录中继续演化。
