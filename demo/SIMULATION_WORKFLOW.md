...
- `event_state.json`：事件在当前时间步的动态状态，每轮读取并更新。
...
读取 `event_state.json`，获得时间步 `t` 的动态状态：

- `event_id`：事件编号。
- `step`：当前时间步。
- `official_statement`：当前已经发布的官方声明。
- `official_statement_status`：官方声明状态，如 `none`、`incomplete`、`clear`、`conflict`。
- `group_conflict`：当前是否出现明显的群体对立。
- `seriousness`：当前事件严重程度，如 `low`、`medium`、`high`。
...
该函数根据本轮的情绪分布、评论派系、互动结果和官方回应，生成 `state(t+1)`。
至少需要保证：
...
生成的新状态写入 `event_state_history.jsonl`，并可覆盖当前运行时使用的 `event_state.json`。
...
| `event_state_history.jsonl` | 每轮事件状态快照 |
...
- `event_state_updater.py`：负责生成下一轮事件状态，不负责保存 Persona。
...
3. 传播和指标统计完成后，才能生成 `event_state(t+1)`。
4. `t+1` 的决策不能读取同一轮尚未完成的中间结果。
